...
@section('tab-title')
<title>{{__('t.Client Person')}}</title>
@endsection
@section('css')
<!-- DataTables -->
...
@endsection
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.Client Person')}}</p>
@endsection
@section('btnTable')
<div class="mb-1">
        ...
        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline"
            ...
            <thead>
                <tr class="listTable">
                    ...
                    <td>{{__('t.name kh')}}</td>
                    <td>{{__('t.name en')}}</td>
                    <td>{{__('t.client_person_position')}}</td>
                    <td>{{__('t.actions')}}</td>
                </tr>
            </thead>
...
<!-- modal -->
@include('options_01.client_person.create')
@include('options_01.client_person.edit')
@endsection
@section('js')
<!-- DataTables  & Plugins -->
...
$(document).ready(function() {
    ...
    $("#menu_client_person").addClass("active");
    <?php 
    ...
    $("#example1").DataTable({
        ...
        columns: [{
            ...
            {
                data: 'clientperson_name_en',
                name: 'clientperson_name_en',
            },
            {
                data: 'clientperson_position',
                name: 'clientperson_position',
            },
            {
                data: 'action',
                data: 'action',
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#e_clientperson_name_kh').val(data.clientperson_name_kh)
                $('#e_clientperson_name_en').val(data.clientperson_name_en)
                $('#e_clientperson_position').val(data.clientperson_position)

...
@section('tab-title')
<title>{{__('t.client_person_type')}}</title>
@endsection
@section('css')
<!-- DataTables -->
...
@endsection
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.client_person_type')}}</p>
@endsection
@section('btnTable')
<div class="mb-1">
...
<!-- modal -->
@include('options_01.client_person_type.create')
@include('options_01.client_person_type.edit')
@endsection
@section('js')
<!-- DataTables  & Plugins -->
...
$(document).ready(function() {
    ...
    $("#Client_Person_Type").addClass("active");
    <?php 
    ...
    $("#example1").DataTable({
        ...
        "searching": false,
        ajax: {
            url: "{{route('client.person.type.index')}}",
            type: 'get',
            data: function(d) {
        ...
        columns: [{
            ...
            {
                data: 'clientpersontype_name_en',
                name: 'clientpersontype_name_en',
            },
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#e_clientpersontype_name_kh').val(data.clientpersontype_name_kh)
                $('#e_clientpersontype_name_en').val(data.clientpersontype_name_en)

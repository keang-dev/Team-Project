...
@section('tab-title')
<title>{{__('t.auditee_person_type')}}</title>
@endsection
@section('css')
<!-- DataTables -->
...
@endsection
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.auditee_person_type')}}</p>
@endsection
@section('btnTable')
<div class="mb-1">
...
<!-- modal -->
@include('options_01.auditee_person_type.create')
@include('options_01.auditee_person_type.edit')
@endsection
@section('js')
<!-- DataTables  & Plugins -->
...
$(document).ready(function() {
    ...
    $("#menu_auditee_person_type").addClass("active");
    <?php 
    ...
    $("#example1").DataTable({
        ...
        "searching": false,
        ajax: {
            url: "{{route('auditee.person.type.index')}}",
            type: 'get',
            data: function(d) {
        ...
        columns: [{
            ...
            {
                data: 'auditeepersontype_name_en',
                name: 'auditeepersontype_name_en',
            },
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#e_auditeepersontype_name_kh').val(data.auditeepersontype_name_kh)
                $('#e_auditeepersontype_name_en').val(data.auditeepersontype_name_en)

...
@section('tab-title')
<title>{{__('t.Auditee Person')}}</title>
@endsection
@section('css')
<!-- DataTables -->
...
@endsection
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.Auditee Person')}}</p>
@endsection
@section('btnTable')
<div class="mb-1">
        ...
        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline"
            ...
            <thead>
                <tr class="listTable">
                    ...
                    <td>{{__('t.name kh')}}</td>
                    <td>{{__('t.name en')}}</td>
                    <td>{{__('t.auditeeperson_position')}}</td>
                    <td>{{__('t.actions')}}</td>
                </tr>
            </thead>
...
<!-- modal -->
@include('options_01.auditee_person.create')
@include('options_01.auditee_person.edit')
@endsection
@section('js')
<!-- DataTables  & Plugins -->
...
$(document).ready(function() {
    ...
    $("#menu_auditee_person").addClass("active");
    <?php 
    ...
    $("#example1").DataTable({
        ...
        "searching": false,
        ajax: {
            url: "{{route('auditee.person.index')}}",
            type: 'get',
            data: function(d) {
        ...
        columns: [{
            ...
            {
                data: 'auditeeperson_name_en',
                name: 'auditeeperson_name_en',
            },
            {
                data: 'auditeeperson_position',
                name: 'auditeeperson_position',
            },
            {
                data: 'action',
                data: 'action',
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#e_auditeeperson_name_kh').val(data.auditeeperson_name_kh)
                $('#e_auditeeperson_name_en').val(data.auditeeperson_name_en)
                $('#e_auditeeperson_position').val(data.auditeeperson_position)

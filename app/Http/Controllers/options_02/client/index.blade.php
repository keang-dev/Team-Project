...
@endsection
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.Client')}}</p>
@endsection
@section('btnTable')
<div class="mb-1">
...
<!-- modal -->
@include('options_02.client.create')
@include('options_02.client.edit')
@endsection
@section('js')
<!-- DataTables  & Plugins -->
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#e_client_name_kh').val(data.client_name_kh)
                $('#e_client_name_en').val(data.client_name_en)
                $('#e_clienttype_id').val(data.clienttype_id)

<div class="modal fade" id="editModal" data-backdrop="static" data-keyboard="false" tabindex="-1"
    ...
    <form action="#" id="formUpdate" class="modal-dialog" method="post" onsubmit="updateForm(event)">
        @csrf
        <input type="hidden" name="entity" value="auditee_person_contacts" required>
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            <label for="auditeeperson_id">{{__('t.auditeeperson_id')}} <span
                                    class="text-danger"></span></label>
                            <select name="auditeeperson_id" id="e_auditeeperson_id" class="form-control" required>
                                <option value="">-----</option>
                                @foreach($auditee_persons as $c)
                                <option value="{{$c->id}}">{{$c->auditeeperson_name_kh}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="auditeepersontype_id">{{__('t.auditeepersontype_id')}} <span
                                    class="text-danger"></span></label>
                            <select name="auditeepersontype_id" id="e_auditeepersontype_id" class="form-control"
                                required>
                                <option value="">-----</option>
                                @foreach($auditee_person_types as $c)
                                <option value="{{$c->id}}">{{$c->auditeepersontype_name_kh}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="auditeecontact_name">{{__('t.auditeecontact_name')}} <span
                                    class="text-danger"></span></label>
                            <input type="text" name="auditeecontact_name" id="e_auditeecontact_name"

...
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.delegation role')}}</p>
    ...
    <div class="col-12">
        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline"
            ...
            <thead>
                ...
                <tr class="listTable">
                    <td>{{__('t.N.')}}</td>
                    <td>{{__('t.delegation_name_kh')}}</td>
                    <td>{{__('t.delegationrole_name_en')}}</td>
                    <td>{{__('t.actions')}}</td>
                </tr>
            </thead>
...
<!-- modal -->
@include('options_01.delegation_role.create')
@include('options_01.delegation_role.edit')
...
$(document).ready(function() {
    ...
    $("#menu_delegation_role").addClass("active");
    ...
    $("#example1").DataTable({
        ...
        "searching": false,
        ajax: {
            url: "{{ route('delegation.role.index') }}",
            type: 'get',
            data: function(d) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#d_delegationrole_name_kh').val(data.delegationrole_name_kh)
                $('#d_delegationrole_name_en').val(data.delegationrole_name_en)

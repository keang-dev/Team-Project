...
@section('tab-title')
<title>{{__('t.Audit Process Status')}}</title>
@endsection
@section('css')
<!-- DataTables -->
...
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.Audit Process Status')}}</p>
...
<!-- modal -->
@include('options_01.audit_process_status.create')
@include('options_01.audit_process_status.edit')
...
$(document).ready(function() {
    ...
    $("#menu_auditprocessstatus").addClass("active");
    <?php 
    ...
    $("#example1").DataTable({
        ...
        "searching": false,
        ajax: {
            url: "{{ route('audit_process_status.index') }}",
            type: 'get',
            data: function(d) {
        ...
        columns: [{
            ...
            {
                data: 'auditprocessstatus_name_en',
                name: 'auditprocessstatus_name_en',
            },
            {
                data: 'auditprocessstatus_color',
                name: 'auditprocessstatus_color',
            },
            {
                data: 'action',
                data: 'action',
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#id').val(data.id)
                $('#e_auditprocessstatus_name_kh').val(data.auditprocessstatus_name_kh)
                $('#e_auditprocessstatus_name_en').val(data.auditprocessstatus_name_en)
                $('#e_auditprocessstatus_color').val(data.auditprocessstatus_color)

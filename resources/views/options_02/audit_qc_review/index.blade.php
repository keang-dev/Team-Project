...
@section('tab-title')
<title>{{__('t.Audit QC Review')}}</title>
@endsection
@section('css')
<!-- DataTables -->
...
@section('titleTable')
<p class="titleTable">តារាងគ្រប់គ្រង{{__('t.Audit QC Review')}}</p>
        ...
        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline"
            ...
            <thead>
                <tr class="listTable text-center">
                    ...
                    <td>{{__('t.auditqc_id')}}</td>
                    <td>{{__('t.audit_qcreview_by')}}</td>
                    <td>{{__('t.audit_qcreview_date	')}}</td>
                    <td>{{__('t.actions')}}</td>
                </tr>
            </thead>
        </table>
...
@include('options_02.audit_qc_review.create')
@include('options_02.audit_qc_review.edit')
...
$(document).ready(function() {
    ...
    $("#menu_audit_qc_review").addClass("active");
    ...
    $("#example1").DataTable({
        ...
        "searching": false,
        ajax: {
            url: "{{ route('audit.qc.review.index') }}",
            type: 'get',
            data: function(d) {
        ...
        columns: [{
            ...
            {
                data: 'auditqc_name_kh',
                name: 'auditqc_name_kh',
            },
            {
                data: 'full_name',
                name: 'full_name',
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status = 200) {
                ...
                $('#e_auditqc_id').val(data.auditqc_id)
                $('#e_audit_qcreview_by').val(data.audit_qcreview_by)
                $('#e_audit_qcreview_date').val(data.audit_qcreview_date)
